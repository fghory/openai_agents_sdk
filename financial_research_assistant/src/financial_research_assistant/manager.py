...
from .agents.financials_agent import financial_agent
...
from .config import config_flash, config_pro
...
class FinancialResearchManager:
    ...
    async def _plan_searches(self, query: str) -> FinancialSearchPlan:
        ...
        result = await Runner.run(planner_agent, f"Query: {query}", run_config=config_pro)
    ...
    async def _search(self, item: FinancialSearchItem) -> str | None:
        ...
        try:
            result = await Runner.run(search_agent, input_data, run_config=config_flash)
    ...
    async def _write_report(self, query: str, search_results: Sequence[str]) -> FinancialReportData:
        ...
        fundamentals_tool = financial_agent.as_tool(
        ...
        # Create a new writer agent with explicit tool configuration
        writer_with_tools = writer_agent.clone()
        writer_with_tools.tools = [fundamentals_tool, risk_tool]
        writer_with_tools.model_settings.tool_choice = "auto"  # Ensure tools are available
        
        self.printer.update_item("writing", "Thinking about report...")
        input_data = f"Original query: {query}\nSummarized search results: {search_results}"
        result = Runner.run_streamed(writer_with_tools, input_data, run_config=config_flash)
        update_messages = [
    ...
    async def _verify_report(self, report: FinancialReportData) -> VerificationResult:
        ...
        result = await Runner.run(verifier_agent, report.markdown_report, run_config=config_flash)
